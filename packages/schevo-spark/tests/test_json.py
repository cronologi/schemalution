...
def test_import_schevo_spark_without_pyspark(monkeypatch: pytest.MonkeyPatch) -> None:
    _block_pyspark(monkeypatch)
    for module_name in list(sys.modules):
        if module_name.startswith("schevo_spark"):
            del sys.modules[module_name]
    module = importlib.import_module("schevo_spark")
